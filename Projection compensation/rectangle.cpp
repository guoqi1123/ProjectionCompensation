...
CvPoint2D32f vertex[4];

int sort_vertex(CvPoint * rect){   //sort the four vertex from left-up, left-down, to right-up, right-down
	CvPoint2D32f temp;
	for(int i = 0;i < 4;i ++)
	{
		vertex[i].x = rect[i].x;
		vertex[i].y = rect[i].y;
	}     //sort the four vertex and find the second largest and smallest value to be the vertex
	for(int i = 0; i < 4; i ++)   //sort x first from small to large
	{
		for(int j = i + 1; j < 4; j ++)
		{
			if(vertex[i].x > vertex[j].x){    //exchange
				temp = vertex[i];
				vertex[i] = vertex[j];
				vertex[j] = temp;
			}
	...
	if(vertex[0].y > vertex[1].y){    //exchange
		temp = vertex[0];
		vertex[0] = vertex[1];
		vertex[1] = temp;
	}
	if(vertex[2].y > vertex[3].y){    //exchange
		temp = vertex[2];
		vertex[2] = vertex[3];
		vertex[3] = temp;
	}
